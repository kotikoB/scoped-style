var pseudoSelectorRegex = /(,?(\s*(::?|(>|~|\+)\s?)(\w|-|_|\.|#|\*|\[([^\[\]])*\]|\(([^\(\)])+\))+)+\s*)+\{\W*[!:;#%\/\\\.\(\)\+,\s\w"'-]+\}/gm;
...
export function mainRule(styles, classID) {
    return (
        ...
        classID +
        '{' +
        styles.replace(commentRegex, '').replace(atRuleRegex, '').replace(pseudoSelectorRegex, '') +
        '}'
    );
}
...
export function pseudoSelectorRules(styles, classID) {
    var rules = [];
    var matches =
        styles.replace(commentRegex, '').replace(atRuleRegex, '').match(pseudoSelectorRegex) || [];
...
export function atRules(styles, classID) {
    ...
    for (var index = 0; index < matches.length; index++) {
        var body = (matches[index].replace(pseudoSelectorRegex, '').match(bodyRuleRegex) || [
            ''
        ])[0];
...
export function globalRules(styles) {
    return (styles.replace(commentRegex, '').match(globalAtRuleRegex) || []).concat(
        styles.replace(commentRegex, '').replace(globalAtRuleRegex, '').match(globalRuleRegex) || []
    );
}
